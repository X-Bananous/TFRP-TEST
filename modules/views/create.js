            ...
            <div class="flex justify-between items-start mb-12 border-b border-white/5 pb-8">
                <div>
                    <div class="inline-flex items-center gap-2 px-3 py-1 bg-blue-500/10 text-blue-400 text-[9px] font-black uppercase tracking-[0.3em] border border-blue-500/20 mb-3 rounded-lg">
                        <i data-lucide="file-text" class="w-3 h-3"></i> Recensement 2025
                    </div>
                    <h2 class="text-4xl font-black text-white uppercase italic tracking-tighter">${isEdit ? 'Mise à jour Dossier' : 'Enregistrement Citoyen'}</h2>
                    <p class="text-gray-500 text-xs font-bold uppercase tracking-widest mt-2">Team French RolePlay • Registre Officiel ERLC</p>
                </div>
            ...
            <form onsubmit="actions.submitCharacter(event)" class="space-y-10">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-3">
                        ...
                        <input type="text" name="first_name" value="${char.first_name || ''}" required placeholder="Ex: Lucas" 
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" name="last_name" value="${char.last_name || ''}" required placeholder="Ex: Martin" 
                            class="glass-input w-full p-4 rounded-2xl text-base font-bold bg-black/40 border-white/10 focus:border-blue-500/50 uppercase tracking-tight">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-3">
                        <label class="text-[10px] font-black text-gray-600 uppercase tracking-widest ml-1 block">Origine (Lieu de naissance)</label>
                        <input type="text" name="birth_place" value="${char.birth_place || 'France'}" required placeholder="Ville, Pays" 
                ...
                <!-- ALIGNMENT SELECTION -->
                <div class="space-y-4">
                    <label class="text-[10px] font-black text-gray-600 uppercase tracking-widest ml-1 block text-center">Secteur d'activité</label>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <label class="cursor-pointer group relative">
                            ...
                            <div class="p-8 rounded-[32px] bg-black/40 border border-white/5 peer-checked:bg-blue-600/10 peer-checked:border-blue-500 peer-checked:shadow-[0_0_30px_rgba(59,130,246,0.2)] hover:bg-white/5 transition-all text-center h-full relative overflow-hidden">
                                ...
                                <div class="text-[10px] text-gray-500 leading-relaxed font-bold uppercase tracking-widest">Voie classique • Service Public • Emploi privé</div>
                            </div>
                        </label>
                        <label class="cursor-pointer group relative">
                            ...
                            <div class="p-8 rounded-[32px] bg-black/40 border border-white/5 peer-checked:bg-red-600/10 peer-checked:border-red-500 peer-checked:shadow-[0_0_30px_rgba(239,68,68,0.2)] hover:bg-white/5 transition-all text-center h-full relative overflow-hidden">
                                ...
                                <div class="text-[10px] text-gray-500 leading-relaxed font-bold uppercase tracking-widest">Risque élevé • Syndicats du crime • Marché Noir</div>
                            </div>
                        </label>
                    </div>
                </div>
                ...
                ${isEdit ? `
                    <div class="bg-orange-600/10 border border-orange-500/30 p-6 rounded-3xl flex gap-5 items-center">
                        ...
                        <p class="text-[11px] text-orange-200 font-bold uppercase tracking-wide leading-relaxed">Note : Toute modification majeure d'état civil annulera votre statut de validation actuel pour examen.</p>
                    </div>
                ` : `
                    <div class="bg-blue-600/5 border border-blue-500/20 p-6 rounded-3xl flex gap-5 items-start">
                        ...
                        <p class="text-[11px] text-blue-100/80 font-bold uppercase tracking-wide leading-relaxed">
                            Ce formulaire de recensement est obligatoire pour évoluer sur le serveur. Une fois transmis, attendez le signalement de la douane.
                        </p>
                    </div>
                `}

                <div class="pt-6 border-t border-white/5 flex justify-end gap-6 items-center">
                    <button type="button" onclick="actions.cancelCreate()" class="text-[10px] font-black text-gray-600 uppercase tracking-[0.2em] hover:text-white transition-colors">Abandonner</button>
                    <button type="submit" class="glass-btn px-12 py-5 rounded-[24px] font-black text-xs uppercase tracking-[0.3em] flex items-center gap-4 bg-blue-600 hover:bg-blue-500 shadow-2xl shadow-blue-900/30 transition-all transform hover:scale-[1.02] active:scale-95">
                        <i data-lucide="save" class="w-5 h-5"></i> ${isEdit ? 'RATIFIER LES MODIFICATIONS' : 'TRANSMETTRE AU REGISTRE'}
                    </button>
                </div>
            </form>
        </div>
